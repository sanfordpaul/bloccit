...
RSpec.describe VotesController, type: :controller do
  ...
  context "guest" do
    describe "POST up_vote" do
      it "redirects the user to the sign in view" do
        post :up_vote, format: :js, params: { post_id: user_post.id }
    ...
    describe "POST down_vote" do
      it "redirects the user to the sign in view" do
        delete :down_vote, format: :js, params: { post_id: user_post.id }
        expect(response).to redirect_to(new_session_path)
      end
    end
  end
  ...
  context "signed in user" do
    ...
    describe "POST down_vote" do
      it "the users first vote increases number of post votes by one" do
        votes = user_post.votes.count
        post :down_vote, format: :js, params: { post_id: user_post.id }
        expect(user_post.votes.count).to eq(votes + 1)
      end
